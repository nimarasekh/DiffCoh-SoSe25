...
\end{example}
For the next example, let $G$ be a simplicial group, $\lB G$ the corresponding simplicial category. Since $G$ is a Kan complex (\cite[Lemma I.3.4]{goerssjardine1999simplicialhomotopytheory}), $BG:=\mN_\Delta\lB G$ is a $\infty$-category, and a $\infty$-groupoid, since $\Ho BG$ is a groupoid. Our notation $BG$ is sensible, since \cite{arakawa2023classifying} shows that $BG$ is equivalent to the delooping of $G$ (constructed using the $\overline{W}$-functor, see \cite{goerssjardine1999simplicialhomotopytheory}). 
