...
\begin{example}
	Let $BO(n)$ be the topological groupoid of $n$-dimensional, inner product spaces and orthogonal morphisms, viewed as an $\infty$-groupoid. Consider the composition
\[
	\begin{tikzcd}
		f_n\colon BO(n) \arrow[r,"\Th_n"] & \sS_* \arrow[r,"\Sigma^{\infty-n}"] & \Sp
	\end{tikzcd}
\]	
where the first functor maps an inner product space $V$ to its one-point compactification. Choosing an orthonormal basis for $V$, we get an isomorphism $\bR^n\simeq V$, which induces an equivalence of spectra
\[\Sigma^{\infty-n}\Th(V)\simeq\Sigma^{\infty-n}\Th(\bR^n)\simeq\Sigma^{\infty-n}S^n\simeq\bS.\] 
In particular, $f_n$ factors through $\Line_\bS$. Let $p:BO(n)\to*$, then 
\[Mf_n\simeq p_!\Sigma^{\infty-n}\Th_n\simeq\Sigma^{\infty-n}p_!\Th_n\simeq\Sigma^{\infty-n}\Th(E_n),\]
