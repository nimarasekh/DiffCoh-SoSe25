...
\end{example}
Given $f:X^{op}\to\Mod_R$ and a sequence $F\xrightarrow{i} X\xrightarrow{\pi} Y$, there is an induced sequence of Thom $R$-module spectra $MF\to MX\to MY$. If $\pi i$ factors through the point, $MF\simeq R\wedge\Sigma^\infty_+F$. 
...
\begin{example}\label{ex:tmf}
	Let $\tmf$ be the $E_\infty$-ring spectrum of topological modular forms and $\sigma:MString\to\tmf$ the $String$-orientation of $\tmf$. The group $String$ is $E_\infty$-monoidal, so $MString$ is a $E_\infty$-ring spectrum. The orientation $\sigma$ is a ring homomomorphism. In the fiber sequence $K(\bZ,3)\to BString\to BSO$, the fiber map $i:K(\bZ,3)\to BString$ is also a monoid homomorphism, so the composition 
	\begin{center}
		\begin{tikzcd}
			\Sigma^\infty_+K(\bZ,3)\arrow[r,"Mi"] & MString \arrow[r,"\sigma"] & \tmf
		\end{tikzcd}
	\end{center}is a homomorphism. Using \cref{lem:adj}, we conclude that the induced homomorphism $K(\bZ,3)\to\Omega^\infty\tmf$ factors through $GL_1(\tmf)$ and so it induces a functor $K(\bZ,4)\to\Line_\tmf$, i.e. \emph{2-bundle gerbes twist $\tmf$}.
\end{example}
...
\end{remark}
\begin{remark}
	$\Sigma^nR$ is invertible, with inverse $\Sigma^{-n}R$. In particular, there is a map $\bZ\times\Line\to\Pic_R$. However, this map is neither injective (if $R$ is $n$-periodic), nor surjective. 
\end{remark}
...
\begin{definition}
	Denote by $M$ the \emph{Thom $R$-module spectrum} functor \[\Grp^{op}_\infty/\Mod_R\to\Mod_R\]sending a functor $f:X^{op}\to\Mod_R$ to its colimit.  
\end{definition}
\begin{example}
