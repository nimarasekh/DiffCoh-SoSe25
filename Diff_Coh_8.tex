...
\begin{definition}[{Thom spectrum}] \label{def:thom}
	...
	\[
	M \colon \Grpd_{\infty}/\Line_R \to \Mod_R,
	\]
	which sends $f\colon X^{\op} \to \Line_R$ to the $R$-module spectrum $\colim(X^{\op} \xrightarrow{f} \Line_R\hookrightarrow \Mod_R)$.
\end{definition}
...
\begin{remark}
	For a given functor $\phi: X \to Y$, we get a pullback functor $\phi^*\colon \Mod_R(Y) \to \Mod_R(X)$ by pre-composition with $\phi^{\op}$. Since $\phi^*$ preserves both limits and colimits, we can construct a 3-terms adjunction \begin{equation}
		\begin{tikzcd}
			\Mod_R(X) \arrow[r,shift right=3,"\phi_*"'] \arrow[r,shift left=3,"\phi_*"] & \Mod_R(Y) \arrow[l,"\phi_!" description]
		\end{tikzcd}
	\end{equation}Explicitly, $\phi_!$, resp. $\phi_*$, is given by left, resp. right, Kan extension along $\phi^\op$. In particular, if $Y=*$ and $\phi=p:X\to*$ is the terminal functor, then $p_!=\colim_{X^\op}$, and so $Mf\simeq p_!(f)$. 
\end{remark}
%--------------TO BE CONTINUED-----------------
