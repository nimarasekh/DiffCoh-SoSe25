...
\[DX \colon = \Map(\Sigma^\infty_+ X, \bS) \]
for the Spanier-Whitehead dual of $X$. 

We can lift $D$ to a functor 
\[ D\colon \sS^{op} \to \Sp \]
...
\begin{itemize}
	\item What is the mapping endo-spectrum of $\bS$?
	\item Is there a map out of $\bS$?
\end{itemize}
...
\[ B \to \Mfd\]
where $\Mfd$ is the $\infty$-category of smooth and proper manifolds, where the $\infty$-categorical structure is given by the usual topological enrichment.

Given such a functor $F$ we have the composition functor 
\[ B^{op} \xrightarrow{ \ F^{op} \ } \Mfd^{op} \xrightarrow{ \ BDiff(-)  \ } \sS^{op} \xrightarrow{ \ \phi \ } \Sp_{\bS/},\]
which is an object in $\Fun(B^{op}, \Sp_{\bS/})$. An object therein is equivalent to a functor $B \to \Sp$ that sits under the constant functor $\bS_B\colon B \to \Sp$	with value $\bS$, meaning we have a natural transformation
\[\phi_{X/B}\colon \bS_B D_B(X) \]
This just unwinds to a natural map of spectra 
\[ \bS \to D(BDiff(X_b))\]
even though the authors write it as 
\[ \bS \to D(X_b) \]
This suggests that the original definition of the functor has issues, which should be fixed later. 

We can now take the left Kan extension along the terminal map $p\colon B \to *$ to get a map of spectra
...
where $T_f$ is the bundle of	tangents along the fibers of $f$ and $X^{-T_f}$, giving us the Thom spectrum.
...
\begin{remark}
	...
	where $J$ is the $J$-homomorphism, and here $R$ is an arbitrary ring spectrum, which we want to us as our cohomology coefficients.
\end{remark}
...
\begin{definition}
	Given a twist $\alpha X \to \Line_R$, an orientation of $X^\alpha$, is a lift 
	\[ 
	\begin{tikzcd}
		& \triv_R \arrow[d, "\pi"] \\ 
...
The left hand side will be $B^\alpha$. The right hand side is equivalent to $X^{-T_f + \alpha f}$, where $\alpha f$ is the composition of $\alpha$ and $f\colon X \to B$.
