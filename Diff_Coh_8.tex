...
\end{proof}
Let $G$ be a topological group and $BG$ the $\infty$-groupoid with a single object $1$ and hom-space $G$. The category $\sS_G\coloneqq\Fun(BG,\sS)$ is equivalent to the category of $G$-spaces.
\begin{lemma}
...
\end{definition}
Let $B$ be a compact space and $\pi\colon X \to B$ a continuous function such that, for every $b\in B$, the fiber $\pi^{-1}(b)=:X_b$ is equipped with the structure of a closed smooth manifold, which varies continuously in $B$, in the sense of being classified by a functor
\[f:B \to \Mfd_\infty\]
...
\[ \Fun(B^{op},\bS/\Sp) \simeq \bS_B/\Fun(B^{op}, \Sp) \](Recall that $\bS_B$ denotes the constant functor $B^{op}\to\Sp$ with value $\bS$), i.e. it is a natural transformation \[\phi_{X/B}:\bS_B\to D_B(f)\]where $D_B$ denotes the Spanier-Whithead duality applied to (the opposite of) $f:B\to\sS$ point-wise. 

We can now take the left Kan extension along the terminal map $p\colon B \to *$ (i.e. take the colimit of these diagrams), to get a map of spectra
\[ p_! \phi_{X/B} \colon p_! \bS_B \to p_! D_B(X) \]
Using the fact that 
\[p_!	\bS_B \simeq  p_! p^* \bS \simeq \Sigma^\infty_+ B,\] 
we get a map of spectra
\[ \Sigma^\infty_+ B \to p_! D_B(X).\]
On the right hand side 
\[p_!	D_B(X) \simeq X^{-T_f} \simeq \Sigma^\infty Th (-T_f)\] 
where $T_f$ is the bundle of	tangents along the fibers of $f$ and $X^{-T_f}$, giving us the Thom spectrum. This last equivalence is a complicated computation.

\begin{remark}
	Here argument involves proving that the classical Thom space $X^{-T_f}$ is equivalent to the colimit of the functor $D_B(X)\colon B \to \Sp$
\end{remark}

\begin{remark}
	Let us justify this notation. Let $X \to BO(d)$ be the classifying map classifying the tangent bundle (as we have fiber-wise tangent bundles, which assemble into a map). Then we get the induced map 
	\[ X \to BO(d) \xrightarrow{ \ J \ }  BGL_1(\bS)\to BGL_1(R) \to \Pic(R)\]
	where $J$ is the $J$-homomorphism, and here $R$ is an arbitrary ring spectrum, which we want to use as our cohomology coefficients.
\end{remark}

These computations give us the \emph{Pontryagin-Thom transfer map}:

\[ PT(f)\colon \Sigma^\infty_+ B \to X^{-T_f} .\]

Now, given a choice of $R$-orientation, we get a Thom isomorphism
\[ R^*(\Sigma^\infty_+X) \xrightarrow{ \ \simeq \ } R^{* - d}(X^{-T_f})\]

We now have the following definition.

\begin{definition}
	With the assumptions as above, the \emph{Umkehr map} is the map: 
	\[ R^*(\Sigma^\infty_+ X) \simeq R^{* - d}(X^{-T_f}) \xrightarrow{ \ PT(f)^{* - d} \ } R^{* - d}(\Sigma^\infty_+ B). \]
\end{definition}
Here it is called the Umkehr map, as it goes the other way from the regular map $R^*f\colon R^*B \to R^*X$.

Recall here the following definition of orientation that we used here. 

\begin{definition}
	Given a twist $\alpha\colon X \to \Line_R$, an orientation of $X^\alpha$, is a lift 
	\[ 
	\begin{tikzcd}
		& \triv_R \arrow[d, "\pi"] \\ 
		X \arrow[r, "\alpha"] \arrow[ur, dashed] & \Line_R 
	\end{tikzcd}
	\]
\end{definition}

We now define a twisted variant of the Umkehr map.

Given a twist $\alpha\colon B \to \Pic_R$, we can smash to get a map 
\[\bS_B \wedge_B \alpha \to D_BX \wedge \alpha \]
The left hand side will compute to $B^\alpha$. The right hand side is equivalent to $X^{-T_f + \alpha f}$, where $\alpha f$ is the composition of $\alpha$ and $f\colon X \to B$.

Now again assuming we have a chosen an $R$-orientation of $T_f$, we get a Thom isomorphism
\[ R^{* + d}(X) \simeq R^*(X^{-T_f + \alpha f}), \]
which gives us 
\[ R^{* + d}(X) \simeq R^*(X^{-T_f + \alpha f}) \to R^*(B^\alpha) = R^{* - \alpha - d} (B)\] 
This map is called the \emph{twisted Umkehr map}.

Let us look at some examples. Assume the following diagram commutes:

\[ 
\begin{tikzcd}
 X \arrow[r, "-T_f"] \arrow[d] & \Pic_\bS \arrow[d] \\ 
	B \arrow[r, "\alpha"] & \Pic_R 
\end{tikzcd}
\]

Then we have $X^{-T_f + \alpha f} \simeq \Sigma^\infty_+ X \wedge R$. 
Then we have an induced map $B^\alpha \to \Sigma^\infty_+ X \wedge R$, but then applying $R^*$ we get the Umkehr map 
