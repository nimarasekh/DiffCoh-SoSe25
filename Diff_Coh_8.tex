...
\section{Umkehr Map}
We now proceed to the construction of the umkehr map in twisted cohomology theories. Here we follow \cite{abg2018thom} 

\begin{remark}
	Depending on which cohomological degrees we want, we sometimes use the following alternative definition of	twisted cohomology:
	\[ R^{* + \alpha}(X) = \pi_*\Map_R(M(\alpha^{-1}), R) \]
 meaning we use the inverse of $\alpha$ to get a more standard degree convention at the level of actual cohomology groups.
\end{remark}

For $X$ a space, we will write 
\[DX \coloneqq \Map(\Sigma^\infty_+ X, \bS) \]
for the Spanier-Whitehead dual of $X$. Notice $D$ is evidently given as the composition functor 
\[ D\colon \sS^{op}  \xrightarrow{ \ \Sigma^\infty_+ \ } \Sp^{op} \xrightarrow{ \ \Map(-,\bS) \ } \Sp. \]
Applying $D$ to the unique map $X \to *$	gives a map of spectra
\[\phi(X)\colon\bS \to DX \]
and we regard this as a functor 
\[\phi\colon \sS^{op} \to \Sp_{\bS/} \]
i.e. the induced functor on the slice category. 

As part of this explanation it is helpful to recall the following facts.

\begin{remark}
	Given a spectrum $E$, the mapping spectrum $\Map(\bS,E)$	is equivalent to $E$ itself. This is because $\bS$ is the unit for the smash product of spectra, and the mapping spectrum from the unit to any spectrum recovers that spectrum. In particular $\Map(\bS,\bS) \simeq \bS$.
\end{remark}

Suppose we are given $f\colon X \to B$, a smooth and proper family of manifolds over $B$, $f^{-1}(b)=X_b$ is a smooth and proper manifold. Where proper means a compact closed manifold. This should vary continuously over $B$ in the sense that $f$ is classified by a functor 
\[ B \to \Mfd\]
where $\Mfd$ is the $\infty$-category of smooth and proper manifolds, where the $\infty$-categorical structure is given by the usual topological enrichment. Notice it comes with an evident forgetful functor to $\sS$, the $\infty$-category of spaces (i.e. $\infty$-groupoids).

Given such a functor $F$ we have the composition functor 
\[ B^{op} \xrightarrow{ \ F^{op} \ } \Mfd^{op} \to \sS^{op} \xrightarrow{ \ \phi \ } \Sp_{\bS/},\]
which is an object in $\Fun(B^{op}, \Sp_{\bS/})$. Notice, there is an evident equivalence of $\infty$-categories
\[ \Fun(B^{op}, \Sp_{\bS/}) \simeq (\Fun(B^{op}, \Sp))_{\bS_B/} \]
where $\bS_B$ is the constant functor with value $\bS$. Thus, our composition above, corresponds to a natural transformation
\[\phi_{X/B}\colon \bS_B \to D_B(X) \]
This just unwinds to a natural map of spectra 
\[ \bS \to D(X_b) \]
...
Using the fact that 
\[p_!	\bS_B \simeq  p_! p^* \bS_B \simeq \Sigma^\infty_+ B,\] 
we get a map of spectra
\[ \Sigma^\infty_+ B \to p_! D_B(X).\]
On the right hand side 
\[p_!	D_B(X) \simeq X^{-T_f} \simeq \Sigma^\infty Th (_-T_f)\] 
...
\begin{remark}
	Here argument involves proving that the classical Thom space $X^{-T_f}$ is equivalent to the colimit of the functor $D_B\colon B \to \Sp$
\end{remark}
