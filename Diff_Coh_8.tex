...
\end{proof}
In particular, $GL_1(R)$ consists of the connected components of equivalences in $\Omega^\infty$. 
\begin{definition}
...
\begin{definition}
	Denote by $j$ the induced $\infty$-group homomorphism $O\to GL_1(\bS)$ induced by the sequence $j_n$, called the \emph{$J$-homomorphism}.  
\end{definition} 
\begin{example}
...
\begin{example}\label{ex:tmf}
	Let $\tmf$ be the $E_\infty$-ring spectrum of topological modular forms and $\sigma:MString\to\tmf$ the $String$-orientation of $\tmf$. The group $String$ is $E_\infty$-monoidal, so $MString$ is a $E_\infty$-ring spectrum and $\sigma$ is a ring homomomorphism. In the fiber sequence $K(\bZ,3)\to BString\to BSO$, the fiber map $i:K(\bZ,3)\to BString$ is also a monoid homomorphism, so the composition 
	\begin{center}
		\begin{tikzcd}
			\Sigma^\infty_+K(\bZ,3)\arrow[r,"Mi"] & MString \arrow[r,"\sigma"] & \tmf
		\end{tikzcd}
	\end{center}is a homomorphism. Using \cref{lem:adj}, we conclude that the induced homomorphism $K(\bZ,3)\to\Omega^\infty\tmf$ factors through $GL_1(\tmf)$, or equivalently, $K(\bZ,4)\to\Line_\tmf$, i.e. \emph{2-bundle gerbes twist $\tmf$}.
\end{example}
...
\end{definition}
If $L$ is a $R$-line, then $\Sigma^nL$ is invertible, with inverse $\Sigma^{-n}L$. Moreover, $\Sigma^nL$ is a line if and only if $R$ is $n$-periodic.
\begin{example}
