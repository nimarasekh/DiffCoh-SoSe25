...
\end{definition}
Denote by $\widehat{\bR}$ the sheaf of {sets} associated to $\bR$ by the Yoneda embedding, mapping a manifold $M$ to the set of smooth maps $M\to\bR$. The sheaf $\widehat{\bR}$ is a ring object in $\Shv(\Mfd,\set)$, a $\widehat{\bR}$-module is called a \emph{smooth module}. Explicitly, a smooth module is a sheaf $V$ together with a natural $C^\infty(M)$-module structure on $V(M)$, for every $M$.
\begin{lemma}[{\cite[Lemma 7.12]{bunkenikolausvoelkl2016diffcoh}}]\label{lem:sheafloc}
  Let $V:\Mfd^{\op}\to\Ch$ a sheaf of chain complexes of smooth modules, then the composition $\Mfd^{\op}\xrightarrow{V}\Ch\to\Dr$ is a sheaf. 
\end{lemma}
...
\end{remark}
The de Rham homomorphism is a cochain homorphism $\Omega^*(M)\to C_\sm^*(M,\bR)$. Restriction to smooth cycles induces a quasi-isomorphism $C^*(M,\bR)\simeq C^*_\sm(M,\bR)$, the de Rham theorem states that the de Rham homomorphism induces an isomorphism of graded rings between de Rham cohomology and smooth singular cohomology with real coefficients, therefore $H_\dr^*(M)\simeq H^*(M,\bR)$, as graded rings. In this section, we rephrase the de Rham homomorphism as a morphism of sheaves valued in $\Dr$ and reference a result generalizing the de Rham theorem.
\begin{remark}\label{rmk:Dcotensor}
  Since $\D(\bZ)$ is presentable, we know that it is cotensored over $\s$. Given a space $S$ and a chain complex $V_*$, the cotensor $V_*{}^S$ is the cochain complex (viewed as a chain complex) of graded linear maps $C_*(S,\bZ)\to V_*$, from the (normalized) singular chain complex of $S$ to $V_*$, see \cite[Definition 1.3.2.1]{lurie2017ha}. In particular, let $V_*=V$ be concentrated in degree 0, then $V_*{}^S$ is the singular cochain complex of $S$ with values in $V$. 
\end{remark}
Recall the functor $\flat$ defined in \cref{def:flat}. By definition of $\Sing:\Mfd\to\s$ and \cref{rmk:Dcotensor}, given $V_*$ chain complex, $\flat V$ maps a manifold $M$ to the cochain complex of graded linear maps $C_*^\sm(M,\bZ)=C_*(\Sing(M),\bZ)\to V_*$. 
\begin{definition}\label{def:derhammorf}
Consider the natural morphism $\Omega^*(M)\to (\flat\bR)(M)=C_\sm^*(M,\bR)$ taking a form $\omega\in\Omega^n(M)$ to the linear map $\int\omega:C^\sm_{n}(M,\bZ)\to\bR$. We call the natural transformation $\dr:\Omega^*\to\flat\bR$ the \emph{de Rham morphism}.  
\end{definition}
\begin{remark}
...
In this section, we give the definition of the $k$-th Deligne sheaf as the Eilenberg-Mac Lane spectrum (see \cref{def:EMsheaf}) associated to a sheaf $V:\Mfd^{\op}\to\Dr$. Following that, we give explicit cochain complexes that are quasi-isomorphic to the value of $V$ at a manifold $M$. 
\begin{definition}\label{def:deligne}
  Given $k\in\bN$, define $\widehat{\bZ}(k):\Mfd^{\op}\to\Dr$ as the pullback 
  \[\begin{tikzcd}
    \widehat{\bZ}(k)\arrow[r]\arrow[d] & \Omega^{\geq k}\arrow[d] \\
    \flat\bZ \arrow[r] & \flat\bR
  \end{tikzcd}\]
  The vertical morphism being the composition $\Omega^{\geq k}\hookrightarrow\Omega^*\xrightarrow{\dr}\flat\bR$. We call the corresponding Eilenberg-Mac Lane spectrum $H\hat{\bZ}(k)$ the $k$\emph{-th Deligne sheaf}.
\end{definition}
\begin{remark}[Model A, see {\cite[\S 3.2]{hopkinssinger2005diffcoh}}]\label{rmk:A} Let $\acute{C}(k)^n(M)\subseteq C_\sm^n(M,\bZ)\oplus C_\sm^{n-1}(M,\bR)\oplus\Omega^n(M)$ consist of triples $(c,h,\omega)$ for which $\omega=0$ if $n<k$, with differential $\delta(c,h,\omega)=(\delta c,\dr(\omega)-c-\delta h,d\omega)$. This complex $\acute{C}^*(k)(M)$ fits into a diagram
\[\begin{tikzcd}
    \acute{C}^*(k)(M) \arrow[d]\arrow[r] & \Omega^{\geq k}(M)\arrow[d] \\
    C_\sm^*(M,\bZ) \arrow[r] & C_\sm^*(M,\bR)
  \end{tikzcd}\]which commutes up to homotopy given by the projections $\acute{C}^n(k)(M)\to C_\sm^{n-1}(M,\bR)$. The diagram above model the pullback of \cref{def:deligne}, hence $\acute{C}^*(k)(M)\simeq\hat{\bZ}(k)(M)$. 
\end{remark}
\begin{remark}[Model B]\label{rmk:B} Recall that $\flat$ preserves cofiber sequences, since it is left adjoint, and that fiber sequences are the same a cofiber sequences in stable $\infty$-categories. Consider the diagram
  \[\begin{tikzcd}
    \hat{\bZ}(k)\arrow[r]\arrow[d] & \Omega^{\geq k}\arrow[d]\arrow[dd,bend left=5em,color=red] \\
    \flat\bZ \arrow[r]\arrow[d] & \flat\bR\arrow[d] \\
    0\arrow[r,color=red] & \flat(\bR/\bZ)
  \end{tikzcd}\]
  Since the bottom square is an pullback, $\hat{\bZ}(k)(M)$ is equivalent to the pullback of the diagram in \textcolor{red}{red}. Let $\breve{C}^{n}(k)(M)\subseteq C_\sm^{n-1}(M,\bR/\bZ)\oplus\Omega^n(M)$ consist of pairs $(\chi,\omega)$ for which $\omega=0$ if $n<k$, with differential $\delta(\chi,\omega)=(e^{2\pi i\dr(\omega)}-\delta\chi,d\omega)$. Similar to \cref{rmk:A}, the complex $\breve{C}^*(k)(M)$ fits into the above diagram so that the outer square is an pullback, hence it is equivalent to $\hat{\bZ}(k)(M)$.
\end{remark}
Take an $n$-cocycle ($n\geq k$) in the model from \cref{rmk:B}, i.e. $(\chi,\omega)\in C_\sm^{n-1}(M,\bR/\bZ)\oplus\Omega^{n}(M)$ such that $d\omega=0$ and $\delta\chi=e^{2\pi i\dr(\omega)}$. Such a cocycle determines a differential character of degree $n-1$ for $M$, in the sense of the following definition:
...
\end{definition}If $\B$ is the sub-site of contractible manifolds, we recover the notion of a \emph{good open cover}. In our next model, we will construct a sheaf $F$ equivalent to $\hat{\bZ}(k)$ on a dense sub-site $\B$. Given a generic manifold $M$, we can evaluate $\hat{\bZ}(k)(M)$ using $F$, if we assume the existence of a $\B$-good open cover. Here we care mainly about $\B=\Euc$ and so we require the following result. 
...
\begin{remark}[Model C, see {\cite[Lemma 7.3.4]{amabeldebrayhaine2021diffcoh}}]\label{rmk:C} Let $\B\subseteq\Mfd$ be a dense sub-site of contractible manifolds, consider the following diagram in the category of {sheaves} on $\B$
  \[\begin{tikzcd}
     \hat{\bZ}(k)|_\B\arrow[d]\arrow[r] & \Omega^{\geq k}|_\B\arrow[d] \arrow[r] & 0\arrow[d,color=red] \\
    \bZ \arrow[r]\arrow[rr,bend right=2.5em,color=red] & \Omega^*|_\B\arrow[r] & \Omega^{\leq k-1}|_\B
  \end{tikzcd}\] The left square is obtained by: $\Circled{1}$ Apply the restriction to $\B$ functor to the pullback diagram of \cref{def:deligne} $\Circled{2}$ Substitute $\flat\bR|_\B$ with $\Omega^*|_\B$ (see \cref{thm:derham}) and $\flat\bZ|_\B$ with $\bZ$ (see \cref{rmk:constsheaf}). Since the right square is a pullback, $\hat{\bZ}(k)|_\B$ is equivalent to the pullback of the diagram in \textcolor{red}{red}. Let $\check{C}^*(k)$ be the sheaf of chain complexes $\bZ\to\Omega^0\to\cdots\to\Omega^{k-1}\to0\to\cdots$, where $\bZ$ is in degree 0 includes into $\Omega^0$ as constant functions. The complex $\check{C}^*(k)$ fits into the above diagram, so that the outer square is an pullback, therefore $\hat{\bZ}(k)|_\B\simeq\check{C}^*(k)$. 
\end{remark}
\begin{remark}[Model C, Continued]Since $\hat{\bZ}(k)$ is equivalent to $\check{C}^*(k)$ restricted to $\B$, consider a manifold $M$ having a $\B$-good open cover $\mathscr{O}$ and let $\I(\mathscr O)$ the closure of $\mathscr{O}$ under finite intersections, then recall
\[
    \hat{\bZ}(k)(M)\simeq\lim_{U\in\I(\mathscr{O})}\hat{\bZ}(k)(U)\simeq\lim_{U\in\I(\mathscr O)} \check{C}^*(k)(U)
  \]
  By rearranging the limit, we see it is equivalent to the limit of the cosimplicial object taking $n$ to the product over all tuples $(U_0,\cdots,U_n)\in\mathscr{O}^{n+1}$ of $\check C^*(k)(U_0\cap\cdots\cap U_n)$. We can then apply \cite[Lemma 7.10]{bunkenikolausvoelkl2016diffcoh} to calculate the limit of this cosimplicial cochain complex as the total cochain complex of \[\check{C}^{n,m}(k):=\prod_{\mathscr{O}^{n+1}}\check{C}^m(k)(U_0\cap\cdots\cap U_n)\]The differential in the $m$-direction simply comes from $\check C^*(k)$. The differential in the $n$-direction is the alternating sum of the morphisms $\delta^i:\check C^{n,m}(k)\to\check C^{n+1,m}(k)$, for $0\leq i\leq n+1$, described in components as follows: Given $\mu\in\check C^{n,m}(k)$, the $(U_0,\cdots,U_{n+1})$-component of $\delta^i\mu$ is the $(U_0,\cdots,\widehat{U_i},\cdots,U_{n+1})$-component of $\mu$ restricted to $U_0\cap\cdots\cap U_{n+1}$, where the hat under $U_i$ means we're removing it from the tuple. 
\end{remark}
