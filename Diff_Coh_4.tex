...
\begin{theorem}\label{thm:stabledk}
...
\begin{lemma}[{\cite[Lemma 7.12]{bunkenikolausvoelkl2016diffcoh}}]\label{lem:sheafloc}
  Let $F:\Mfd^{op}\to\Ch(\bZ)$ a sheaf of chain complex of $C^\infty$-modules, then $\Mfd^{op}\xrightarrow{F}\Ch(\bZ)\to\D(\bZ)$ is a sheaf. 
\end{lemma}
...
\end{definition}\label{def:EMsheaf}
...
% Last ingredient to Deligne: Equivalence of $\Omega^*$ and $\bR$. 
...
% What does it mean for a sheaf of spectra to refine a spectrum. 
