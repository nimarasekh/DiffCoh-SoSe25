...
\maketitle
	...
	\end{definition}
	\begin{definition}
		Given a pre-sheaf $F\colon\C^{\op}\to\set$, denote by $\C_{/F}$ the category of pairs $(C,c)$, consisting of an object $C$ and an element $c\in F(C)$. A morphism $(D,d)\to(C,c)$ consists of a morphism $f\colon D\to C$ such that $F(f)(c)=d$. 
	\end{definition}By Yoneda's lemma, an element $c\in F(C)$ corresponds to a natural transformation $y(C)\rightarrow F$. Taking the colimit over $(C,c)\in\C_{/F}$ of the pre-sheaves $y(C)$, we get a natural transformation $\colim_{(C,c)\in\C_{/F}}y(C)\rightarrow F$. 
	...
	\end{theorem}
	\begin{definition}
		Let $\C$ be a category. A \emph{sieve} on an object $C$ is a sub-functor of $y(C)$, with $y(C)$ being the \textit{maximal sieve}. Given a morphism $f\colon D\to C$ and a sieve $S$ on $C$, denote by $f^*(S)$ the \textit{pullback sieve}, mapping $X$ to the set of arrows $g\colon X\to D$ such that $fg\in S(X)$. 
	\end{definition}
	...
	\begin{definition}
		...
		is an isomorphism. Explicitly, an object in $\Map(\C_{/S},F)$ consists of an element $x_f\in F(D)$, for every map $f\in S$ (where $D$ is the domain of $f$), such that $x_{fg}=F(g)(x_f)$, for every $f$ and $g$ composable. The canonical morphism takes $x\in F(C)$ to the tuple with $x_f:=F(f)(x)$. 
	\end{definition}Using \cref{thm:density} and Yoneda's lemma, we have the following chain of natural isomorphisms:
	\begin{align*}
		\Map(\C_{/S},F) &= \textrm{lim}_{(D,d)\in\C_{/S}}F(D) \\
		&\simeq \textrm{lim}_{(D,d)\in\C_{/S}}\Nat_\C(y(D),F) \\
		&\simeq \Nat_\C(\colim_{(D,d)\in\C_{/S}}y(D),F)\\
		&\simeq \Nat_\C(S,F)
	...
	\begin{corollary}
		...
		\[ F(C)  \simeq \Nat_\C(y(C),F)\to \Nat_\C(S,F) \]
		is an isomorphism. 
	\end{corollary}
...
We continue our analysis of sheaves with the aim of providing explicit descriptions. Now we focus on the case of sheaves on the site of manifolds. Recall that a sieve $S$ on a manifold $M$ is a covering sieve if an open cover $\rO$ of $M$ exists, such that every morphism in $S$ factors through the inclusion $U\subseteq M$ of some element $U\in\rO$. From the definition of the topology on $\Mfd$ we have the following: Given a covering sieve $S$ generated by an open cover $\rO$. Denote by $\I(\rO)$ the poset (viewed as a category) consisting of finite intersections of elements of $\rO$.
\begin{lemma}
	The functor $i:\I(\rO)\rightarrow\Mfd_{/S}$ is cofinal.
\end{lemma}
In particular, since $\Map(\Mfd_{/S},F)$ is defined as the limit $(\Mfd_{/S})^{\op}\to\Mfd^{\op}\xrightarrow F\s$ and $i^{\op}$ is \emph{final}, there is a canonical equivalence \[\Map(\Mfd_{/S},F)\simeq\lim(\I(\rO)^{\op}\to\Mfd^{\op}\xrightarrow F\s)=\textrm{lim}_{U\in\I(\rO)}F(U)\]
\begin{proposition}\label{pro:sheaf}
  A pre-sheaf $F\colon \Mfd^{\op} \to \Sp$ is a sheaf if and only if for every manifold $M$ and every open cover $\rO$ of $M$, the canonical morphism $F(M) \to \lim_{U \in \I(\rO)}F(U)$ is an equivalence.
\end{proposition}
...
Let $\C$ be a 1-site and $\rP_\Delta(\C)=\Fun(\C^{\op},\sset)$ the ordinary category of simplicial pre-sheaves on $\C$. Given a pre-sheaf $F:\C^{\op}\to\set$, denote by $F_\disc$ the simplicial pre-sheaf mapping $C$ to the constant simplicial set with value $F(C)$. 
