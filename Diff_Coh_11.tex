...
Naively, given an $\infty$-topos $\sG$ with cohesive structure and $X,A$ objects in $\sG$, we would like to define the non-abelian cohomology as 
\[ H^n(X; A) := \pi_0 \mathrm{Map}_{\sG}(X, \mathrm{B}^n A). \]
assuming $A$ admits a delooping $\mathrm{B}^n A$ in $\sG$.
...
\begin{tikzcd}[row sep=1cm, column sep=2cm]
 \hat{E} \arrow[r] \arrow[d] & Z \arrow[d, "\text{homotopification}"] \\ 
	E \arrow[r, "\text{characteristic map}"] & \sH(Z)
\end{tikzcd} 
\]
...
\begin{definition}
 A cohesive	$\infty$-topos $\sG$ is an $\infty$-topos together with an adjoint quadruple
	\[	\Pi \dashv \Disc \dashv \Gamma \dashv \Codisc,	\]
	where $\Gamma\colon \sG \to \sS$ is the global sections functor,	such that
	\begin{enumerate}
...
\begin{example}
	...
	\begin{itemize}
		...
		\item $\Disc$ is given by regarding an $\infty$-groupoid as a constant sheaf,
		\item $\Gamma$ is given by taking global sections,
		\item $\Codisc$ is given by regarding an $\infty$-groupoid as a codiscrete sheaf.
	\end{itemize}
\end{example}
...
\begin{definition}
	Let $\sG$ be a cohesive $\infty$-topos and $A	\in \sG$. The \emph{de Rham complex} of $A$ is defined as
	\[
	\begin{tikzcd}
	 A \arrow[d] \arrow[r] & * \arrow[d] \\
		\Disc \circ \Pi(A) \arrow[r] &  \Pi_{dR}A
	\end{tikzcd}	
	\]
...
\[ 
\begin{tikzcd}
	\sG \arrow[r, shift left=2, "\Pi_{dR}", "\bot"'] & \sG \arrow[l, shift left=2, "\flat_{dR}"]
\end{tikzcd}
\]
...
\begin{definition}
	For objects $X,A$ in a cohesive $\infty$-topos $\sG$ the \emph{differential non-abelian cohomology} of $X$ with coefficients in $A$ is defined as the set
...
\begin{proposition} \label{prop:cohomologycomputation}
 Let $X$ be a sheaf of manifolds. Then 
	\[ \Map(X,\flat_{dR}	\mathbf{B}^n U(1)) \simeq 
	\begin{cases}
...
\begin{proof}[Idea]
	...
	\[\Map(X, \flat_{dR} B^n	U(1)) \simeq \Map(C(\{U_i\}), \Phi(\Omega^1(-) \to \Omega^2(-) \to ... \Omega^n_{cl})), \]
...
\begin{definition} \label{def:curvaturemap}
	Let $A$ be an object in $\sG$, such that $B^{n+1} A$ exists. The \emph{curvature map} is	defined as the map pullbacks
	\[ 
	\begin{tikzcd}
		B^nA \arrow[r] \arrow[d, "\mathrm{curv}"] & * \arrow[d]  \\
		\flat{dR} B^{n+1} A \arrow[r]  \arrow[d] & \Disc \circ \Gamma  B^{n+1} A \arrow[d] \\ 
		* \arrow[r] & B^{n+1} 
	\end{tikzcd}
	\]
\end{definition}
...
\begin{theorem}
	...
	\[ 
		\begin{tikzcd}
			...
			\Map_{sG}(X,B^nA) \arrow[r, "\mathrm{curv}_*"] & \Map_{sG}(X, \flat_{dR} B^{n+1} A)
		\end{tikzcd},
		\]
