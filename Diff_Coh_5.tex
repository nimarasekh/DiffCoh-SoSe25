...
\begin{definition}
  A pre-sheaf $F$ is called \emph{$\mathbb{R}$-invariant} if for any manifolds $M$, the projection $M\times\bR\to M$ induces an equivalence $F(M)\to F(M\times\bR)$. Denote by $\PSh_{\bR}(\Mfd,\C)$ the full sub-category of $\bR$-invariant pre-sheaves, and $\Shv_\bR(\Mfd,\C)$ the full sub-category of $\bR$-invariant sheaves. 
\end{definition}
Note $\mathbb{R}$-invariant pre-sheaves are closed under limits and colimits. Therefore, by the adjoint functor theorem, we conclude the following:
\begin{theorem}
The inclusion $\PSh_\bR(\Mfd,\C)\to\PSh(\Mfd,\C)$ has a right adjoint $R_\bR$ and left adjoint $L_\bR$. The inclusion $\Shv_\bR(\Mfd,\C)\to\Shv(\Mfd,\C)$ has a right adjoint $R_\hi$ (the restriction of $R_\hi$ to sheaves) and $L_\hi$ (the restriction of $L_\hi$ to sheaves, followed by sheafification).  
\end{theorem}
\begin{remark}
  While for a general presentable $\infty$-category $\C$, the functor $L_\bR$ need not preserve sheaves, it does for $\C=\Sp$, see \cite[Remark 4.4.13]{amabeldebrayhaine2021diffcoh}. 
\end{remark}
Let $\C$ be pointed, i.e. having a zero object.
%---------------TO BE CONTINUED------------------
\begin{definition}
