...
\newcommand{\BDelta}{\bm{\Delta}} % simplex category
...
We introduce some notations, definitions, and constructions regarding simplicial sets. For more details, see \cite{goerssjardine1999simplicialhomotopytheory}. For each natural number $n$, we denote by $[n]$ the linearly ordered set $\{0,\cdots,n\}$. Let $\BDelta$ denote the category of linearly ordered sets $[n]$ and (non-strict) monotone functions between them. The following notations/nomenclature will be used throughout:
\begin{itemize}
	\item Given a category $\lC$, a \emph{simplicial}, resp. \emph{cosimplicial}, \emph{object} consists of a functor $\BDelta^\op\to\lC$. resp. $\BDelta\to\lC$. Simplicial objects in $\set$ are called \emph{simplicial sets}. The category of simplicial sets is denoted as $\sset$. 
	\item Given a simplicial set $S$, we write $S_n$ for $S([n])$. Elements of $S_n$ are called \emph{$n$-simplices of $S$}. 
	\item Given $n$, we write $\Delta^n$ for the simplicial set represented by $[n]$. Explicitly, a $k$-simplex of $\Delta^n$ is a monotone function $[k]\to[n]$. Appealing to Yoneda's Lemma, we identify $n$-simplices of $S$ with morphisms of simplicial sets $\Delta^n\to S$. 
	...
	\item Given $n$, a simplicial set $S$, and a $0$-simplex $X$, we'll abuse notation and denote by $X$ the image of $X$ under the map $S_0\to S_n$ induced by the terminal function $[n]\to[0]$.  
	\item Simplicial sets are connected to topological spaces via the singular simplicial set and geometric realization functors. Given $n$, denote by $|\Delta^n|$ the sub-space $\subseteq[0,1]^{n+1}$ of tuples $(t_0,\cdots,t_n)$ such that $t_0+\cdots+t_n=1$. The space $|\Delta^n|$ is called \emph{geometric $n$-simplex}. Given a topological space $X$, denote by $\Sing X$ its \emph{singular simplicial set}, the $n$-simplices of which are continuous maps $|\Delta^n|\to X$. On the other hand, given a simplicial set $S$, denote by $|S|$ its \emph{geometric realization}. 
	\item Simplicial sets are connected to categories via the nerve functor. Given $n$, denote by $[n]$ the category associated to the linearly ordered set $[n]$. Given a category $\lC$, denote by $\mN\lC$ its \emph{nerve}, the $n$-simplices of which are functors $[n]\to\lC$.
\end{itemize}
\begin{definition}
...
\begin{definition}
	A simplicial set $S$ is called a \emph{$\infty$-category} if the following condition holds: For every $0< i< n$ and solid diagram \begin{equation}\label{eq:diag}
    \begin{tikzcd}
...
\end{definition}
\begin{example}
  The nerve of a category is an $\infty$-category. In fact, an $\infty$-category is (isomorphic to) the nerve of a ordinary category if and only if, for every solid diagram such as (\ref{eq:diag}), there is a \emph{unique} dashed arrow making the diagram commute (\cite[Proposition 1.1.2.2]{lurie2009htt}). 
\end{example}
We recall briefly the construction of the \emph{homotopy category} $\Ho S$ of a $\infty$-category $S$, details can be found in \cite{lurie2009htt,kerodon2026homcat}:
\begin{itemize}
	\item Objects of $\Ho S$ are the 0-simplices of $S$.
	\item 
\end{itemize}
\newpage
. The objects of $\Ho S$ are the vertices of $S$. Given a vertex $X$, denote by $\id_X$ the image of $X$ under $s_0:S_0\to S_1$. Given two vertices $X,Y$ and edges $f,g:X\to Y$, we say $f$ is \emph{left homotopic} to $g$ if there is a 2-simplex with the following boundary: 
\begin{equation}
    \begin{tikzcd}
      & Y \arrow[dr,"\id_Y"] & \\
