...
\begin{definition}
	A $\infty$-category $\lC$ is \emph{$\kappa$-filtered} if every diagram indexed by a $\kappa$-small $\infty$-category has a cocone. The colimit of a diagram indexed by a small $\kappa$-filtered $\infty$-category is called a \emph{$\kappa$-filtered colimit}. 
\end{definition}
\begin{definition}
...
\end{definition}
\begin{remark}
	$\kappa$-accessible functors are called \emph{$\kappa$-finitary functors} in \cite{kerodon2026} (see Tag 05Y6). 
\end{remark}
\begin{definition}
...
\begin{definition}
	...
	\begin{enumerate}
		...
		\end{equation}
	\end{enumerate}
	Given a $\kappa$-accessible functor $G:\lD\to\lX$ and functor $H:\lC\to\lX$, an equivalence $\eta:H\to GF$ is said to \emph{exhibit $G$ as the $\Ind_\kappa$-extension of $H$}.  
\end{definition}The following is \cite[Tag 0653]{kerodon2026}. 
\begin{theorem}\label{thm:condind}
	...
	\begin{enumerate}
		\item The $\infty$-category $\lD$ admits small $\kappa$-filtered colimits. 
		\item The functor $F$ is fully faithful. 
		\item For every object $X\in\lC$, its image $F(X)$ is $\kappa$-compact. 
...
\end{definition}
\begin{definition}
	A functor of $\infty$-categories $F:\lC\to\lD$ is called a \emph{localization} if it admits a fully faithful left adjoint. If the left adjoint to $F$ is ($\kappa$-)accessible, we call $F$ a ($\kappa$-)\emph{accessible localization}.
\end{definition}
\begin{theorem}
...
\begin{theorem}\label{thm:locpres}
	Let $\lD\subseteq\lC$ be a full sub-category such that the inclusion $G$ is $\kappa$-accessible and admits a left adjoint $F:\lC\to\lD$. If $\lC$ is presentable, then $\lD$ is presentable. 
\end{theorem}
\begin{lemma}\label{lem:imofcompact}
	Under the hypotheses of Theorem \ref{thm:locpres}, the image under $F$ of a $\kappa$-compact object is $\kappa$-compact. 
\end{lemma}
\begin{proof}
	By adjunction, $\hom_{\lD}(F(X),-)$ is equivalent to $\hom_{\lC}(X,G(-))$, which preserves small $\kappa$-filtered colimits if $X$ is $\kappa$-compact (since $G$ is $\kappa$-accessible). 
\end{proof}
\begin{lemma}\label{lem:indcompletion}
	Under the hypotheses of Theorem \ref{thm:locpres}, let $\lX$ be a small $\infty$-category and $F:\lX\to\lC$ exhibit $\lC$ as the $\Ind_\kappa$-completion of $\lX$. Let $\lY$ be the full sub-category of $\lD$ consisting of the images under $F$ of objects in (the image of) $\lX$, then the inclusion $\lY\subseteq\lD$ exhibits $\lD$ as the $\Ind_\kappa$-completion of $\lY$.  
\end{lemma}
\begin{proof}
	We wish to apply Theorem \ref{thm:condind}. Condition (2) holds by definition of $\lY$. By \cite[Tag 04JW]{kerodon2026}, we know $\lD$ is cocomplete, so condition (1) holds. Lemma \ref{lem:imofcompact} implies that condition (3) holds. Since $G$ is fully faithful, the counit $FG(X)\to X$ is an equivalence (see \cite[Tag 02FF]{kerodon2026}). Finally, sub-category generated by $\lY$ under small $\kappa$-filtered colimits contains the essential image of $F$ (since $F$ is cocontinuous), which is $\lD$, so conditions (4) holds.
\end{proof}
\begin{proof}[Proof of Theorem \ref{thm:locpres}] By \cite[Tag 04JW]{kerodon2026}, we know $\lD$ is cocomplete. By Lemma \ref{lem:indcompletion}, $\lD$ is the $\Ind_\kappa$-completion of a small $\infty$-category, therefore $\lD$ is $\kappa$-accessible.
\end{proof}
\begin{definition}
	Given an $\infty$-category $\lC$ and a class of morphisms $S$ and a functor $F:\lC\to\lD$, we say \emph{$F$ exhibits $\lD$ as the localization of $\lC$ at $S$} if the following hold: 
	\begin{enumerate}
