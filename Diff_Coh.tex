...
\begin{itemize}
    ...
    \begin{tikzcd}
      X \arrow[d,"f"']\arrow[r] & Y\arrow[d,"g"]  \\
      Z\arrow[ru,dashed]\arrow[r] & W
    \end{tikzcd}
  \end{equation}there is a dashed arrow making the diagram commute if $f$, resp. $g$, is a cofibration, resp. fibration, and either one is a weak equivalence. 
...
\end{itemize}
If a pair $f,g$ satisfies the condition that for every diagram like (\ref{diag:lifting}), there is a dashed arrow making the diagram commute, we say $g$, resp. $f$, has the \emph{right lifting property}, resp. \emph{left lifting property}, against $f$, resp. $g$. The last three axioms are equivalent to saying that acyclic cofibration with fibrations, and cofibrations with acyclic fibrations, form a \emph{weak factorization system} (\cite[Definition 11.2.1]{riehl2014categoricalhomotopytheory}), see \cite[\S 11.3]{riehl2014categoricalhomotopytheory}. 
...
\end{itemize}The sets $S_n$, for every $n\geq0$, together with face and degeneracy maps, completely determine a simplicial set, see \cite[Tag 04FW]{kerodon2026} (see also \cite[\S VII.5, Proposition 2]{maclane1998categories}, for a vintage reference). 
\begin{remark}
...
\begin{itemize}
	...
	\item Given 1-simplices $f:X\to Y,g:Y\to Z$, there is a 1-simplex $h:X\to Z$, unique up to homotopy, such that $h$ is a composition of $f$ and $g$ (\cite[Tag 0043]{kerodon2026}). Moreover, the homotopy class of $h$ only depends on the homotopy classes of $f$ and $g$ (\cite[Tag 0048]{kerodon2026}). Therefore, there is a unique function \begin{equation}
		...
		\end{tikzcd}
	\end{equation}  such that $[h]=[g]\circ[f]$ whenever $h$ is a composition of $f$ and $g$. 
	\item The composition laws $\circ_{X,Y,Z}$ are associative and, given a 0-simplex $X$, the homotopy class of $X$ (viewed as a 1-simplex) acts as the identity morphism of $X$ (\cite[004B]{kerodon2026}).
\end{itemize}The following is a special case of \cite[Corollary 1.2.13]{land2021introduction}. 
...
\end{definition}
It can be checked directly that $\star$ underlies a monoidal structure on $\sset$, with monoidal unit $\emptyset$ (see \cite[Tag 016K]{kerodon2026}). 
\begin{definition}
Given a simplicial set $X$, denote by $\iota_X$ the initial functor $\emptyset\to X$ and, given a simplicial set $S$, denote by $\iota_X^\ell$, resp. $\iota_X^r$, the composition \begin{equation}
	\begin{tikzcd}
		S\cong S\star\emptyset \arrow[r,"S\star\iota_X"] & S\star X
	\end{tikzcd}\text{, resp.}\qquad
	\begin{tikzcd}
		S\cong \emptyset\star S \arrow[r,"\iota_X\star S"] & X\star S
	\end{tikzcd}
\end{equation}
The construction $S\star -$, resp. $-\star S$, induces a functor $\sset\to\sset_{S/}$ sending $X$ to $\iota^\ell_X$, resp. $\iota^r_X$.	
\end{definition}

\begin{definition}
	Given a morphism of simplicial sets $\alpha:S\to X$, denote by $X_{\alpha/}$, resp. $X_{/\alpha}$, the simplicial set characterized as follows: A $n$-simplex of $X_{\alpha/}$, resp. $X_{/\alpha}$, is a commutative triangle \begin{equation}
		\begin{tikzcd}[row sep=1cm]
			& S \arrow[dl,"\iota^\ell_{\Delta^n}"']\arrow[dr,"\alpha"]  \\
			S\star\Delta^n \arrow[rr] & & X
		\end{tikzcd}\text{, resp.}\qquad
		\begin{tikzcd}[row sep=1cm]
			& S \arrow[dl,"\iota^r_{\Delta^n}"']\arrow[dr,"\alpha"] \\
			\Delta^n\star S \arrow[rr] & & X
		\end{tikzcd}
	\end{equation}
\end{definition}
\begin{theorem}
...
\section{Limits in Higher Category Theory}

\section{Accessible and Presentable Categories}

% Bla Bla, adjoint functor theorem, presentable categories, bla bla... Definition of regular cardinal. 
Let $\kappa$ be a regular cardinal. A $\infty$-category $\lI$ is \emph{$\kappa$-filtered} if every diagram in $\lI$ has a cocone when indexed by a $\infty$-category with $(<\kappa)$-many simplices. The colimit of a diagram indexed by a $\kappa$-filtered $\infty$-category is called a \emph{$\kappa$-filtered colimit}.   
\begin{definition}
  A functor $F:\lC \to \lD$ is \emph{$\kappa$-accessible} if it preserves $\kappa$-filtered colimits. Given an $\infty$-category $\lC$, an object $X$ is \emph{$\kappa$-compact} if $\hom_{\lC}(X,-):\lC\to\lS$ is $\kappa$-accessible.
\end{definition}
\begin{definition}
  An $\infty$-category $\lC$ is \emph{$\kappa$-accessible} if there exists a set $S\subseteq\lC_0$ of $\kappa$-compact objects that generate $\lC$ under $\kappa$-filtered colimits. An $\infty$-category is \emph{accessible} if it is $\kappa$-accessible, for some regular cardinal $\kappa$. 
\end{definition}
\begin{definition}
	An $\infty$-category $\lC$ is \emph{presentable} if accessible and cocomplete. 
\end{definition}
