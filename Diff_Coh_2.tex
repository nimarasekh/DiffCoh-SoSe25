...
\end{definition}
\begin{example}
  Let $\lC$ be a category. The \emph{nerve of $\lC$}, denoted $\mN\lC$, is the simplicial set where the $n$-simplices are $\Hom_{\cat}(\langle n\rangle,\lC)$. This defines a functor $\mN:\cat \to \sset$.
\end{example}
\begin{proposition}\label{prop:nerve}
  $\mN\lC$ is a quasi-category, and a Kan complex if and only if $\lC$ is a groupoid. 
\end{proposition}
\begin{proof}
Straightforward combinatorics. 
\end{proof}
\begin{remark}
  The nerve is a special case of the following construction: Let $\lC$ be a category, $\Gamma:\BDelta\to\lC$ a functor, then define $\mN_\Gamma$ as the composition \begin{equation}
    \begin{tikzcd}
      \lC \arrow[r] & \Hom_{\cat}(\lC^{op},\set) \arrow[r] & \sset
    \end{tikzcd}
...
\end{remark}
\begin{remark}
  Denote by $\scat$ the category of simplicially enriched categories. In \cite[1.1.5.1]{lurie2009htt}, Lurie constructs a cosimplicial object $\BDelta\to\scat$. The resulting nerve functor $\mN_\Delta:\scat\to\sset$ is called \emph{homotopy coherent nerve}. If $\lC$ is a category enriched over $\infty$-groupoids, its homotopy coherent nerve is a $\infty$-category. The induced right adjoint is denoted $\kC$, the adjoint pair $(\kC,\mN_\Delta)$ underlies a Quillen equivalence. 
\end{remark}
...
\begin{definition}
  $\lS:=\mN_\Delta(\Kan)$ is called the \emph{quasi-category of $\infty$-groupoids}. 
\end{definition}
...
\begin{definition}
  Let $X$ be a quasi-category. The \emph{quasi-category of simplicial presheaves} is defined as $\Fun(X^\op,\lS):=\Hom_{\sset}(X^{\op},\lS)$.
\end{definition}
\begin{theorem}[Yoneda]
  Given a quasi-category $X$, the mapping space functor $X^\op\times X\to\lS$ adjoints to a functor $y:X\to\widehat{X}:=\Fun(X^\op,\lS)$, called the \emph{Yoneda embedding}. Given a cocomplete quasi-category $\lC$, pre-composition by $y$ induces an equivalence \begin{equation}
    \begin{tikzcd}
      \Fun^\mL(\widehat{X},\lC) \arrow[r] & \Fun(X,\lC)
    \end{tikzcd}
  \end{equation}where $\Fun^\mL(\widehat{X},\lC)$ denotes the category of colimit preserving functors $\widehat{X}\to\lC$. 
\end{theorem}
...
\begin{theorem}\label{thm:locpresquasi}
  A quasi-category $X$ is locally presentable (cocomplete and accessible) if and only if there is a small sub-quasi-category $S$ such that $X\to\Fun(S^\op,\lS)$ is a fully faithful, accessible right adjoint. 
\end{theorem}
\begin{remark}
...
\begin{proposition}
  Let $\lD$ be a pointed $\infty$-category. Evaluation at the 0-sphere $S^0$ induces an equivalence \begin{equation}
    ...
    \end{tikzcd}
  \end{equation}where $\Fun^\mL(\lS_*,\lC)$ denotes the category of colimit preserving functors $\lS_*\to\lC$. 
\end{proposition}
...
\end{definition}
In homological algebra, the derived category $\lD(\bA)$ of an abelian category $\bA$ underlies the structure of a triangulated category. In higher category theory, the derived category $\lD_\infty(\bA)$ is constructed as the higher categorical localization of chain complexes at quasi-isomorphisms, then $\Ho\lD_\infty(\bA)\simeq\lD(\bA)$ and the triangulated structure on $\lD(\bA)$ is a reflection of $\lD_\infty(\bA)$ being a stable $\infty$-category. 
